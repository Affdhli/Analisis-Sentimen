...
def implementasi_sistem():
    ...
    tab1, tab2, tab3 = st.tabs(["Analisis Teks", "Analisis File", "Kelola Model"])
...
def _implementasi_analisis_teks():
    ...
    if 'model_package' in st.session_state:
        ...
        st.info("Model dari training session tersedia")
    else:
        # Coba muat model dari file
        model_package = None
        st.warning("Belum ada model yang dimuat dari session state")
    
    # Upload model terbaru
    st.subheader("Upload Model Terbaik (.pkl)")
    
    uploaded_model = st.file_uploader(
        "Upload file model terbaik dalam format .pkl",
        type=['pkl'],
        key="upload_model_for_analysis"
    )
    
    if uploaded_model is not None:
        try:
            # Simpan file sementara
            temp_file = "temp_uploaded_model.pkl"
            ...
            # Validasi struktur model package
            required_keys = ['model', 'tfidf_vectorizer', 'model_info']
            if all(key in model_package for key in required_keys):
                # Simpan ke session state
                ...
                col1, col2, col3, col4 = st.columns(4)
                with col1:
                    st.metric("Nama Model", model_info.get('model_name', 'SVM'))
                with col2:
                    st.metric("Rasio", model_info.get('ratio', 'N/A'))
                with col3:
                    st.metric("Kernel", model_info.get('kernel', 'N/A'))
                with col4:
                    st.metric("Akurasi", f"{model_info.get('accuracy', 0):.4f}")
                
                # Tampilkan detail lengkap
                with st.expander("Detail Model Lengkap"):
                    st.write("**Parameter Model:**")
                    st.json(model_info)
                    
                    # Tampilkan fitur penting jika ada
                    ...
                    st.write("**Informasi Vectorizer:**")
                    st.write(f"- Jumlah fitur: {len(vectorizer.get_feature_names_out())}")
                    st.write(f"- N-gram range: {vectorizer.ngram_range}")
                    st.write(f"- Max features: {vectorizer.max_features}")
                    
                    # Preview model object
                    st.write("**Struktur Model Package:**")
                    package_keys = list(model_package.keys())
                    st.write(f"Keys dalam package: {package_keys}")
                
                # Hapus file sementara
                os.remove(temp_file)
            else:
                st.error("Format file model tidak valid. Pastikan file berisi model package dengan struktur yang benar.")
                os.remove(temp_file)
    ...
    if not model_available:
        ...
        **Model belum tersedia!** 
        ...
        2. Upload model yang sudah ada di tab **"Kelola Model"**, atau
        3. Upload file model .pkl di atas
        """)
        return
    ...
    with st.spinner("Menganalisis sentimen..."):
        try:
            ...
            if 'model_info' in model_package:
                ...
                st.error("Informasi model tidak tersedia untuk fallback")
...
def _tampilkan_hasil_analisis_aktual(text, prediction, confidence, model_package):
    ...
    with st.expander("Detail Analisis", expanded=True):
        ...
        st.write(f"- Teks setelah cleaning: `{cleaned_text[:100]}...`" if len(cleaned_text) > 100 else f"- Teks setelah cleaning: `{cleaned_text}`")
        st.write(f"- Teks tanpa stopwords: `{text_no_stopwords[:100]}...`" if len(text_no_stopwords) > 100 else f"- Teks tanpa stopwords: `{text_no_stopwords}`")
        
        if 'model_info' in model_package:
            model_info = model_package['model_info']
            st.write("**Informasi Model:**")
            st.write(f"- Model: {model_info.get('model_name', 'SVM')} dengan kernel {model_info.get('kernel', 'N/A')}")
            st.write(f"- Rasio training: {model_info.get('ratio', 'N/A')}")
            st.write(f"- Akurasi model: {model_info.get('accuracy', 0):.4f}")
...
def _implementasi_analisis_file():
    ...
    if 'model_package' not in st.session_state:
        ...
        **Model belum tersedia!** 
        ...
        Silakan lakukan salah satu dari berikut:
        1. Lakukan training model di section **"7. Training & Evaluasi SVM"**, atau
        2. Upload model yang sudah ada di tab **"Analisis Teks"**, atau
        3. Upload file model .pkl di tab **"Analisis Teks"**
        """)
        return
    ...
    # Tampilkan informasi model
    st.info(f"Model yang digunakan: {model_package['model_info'].get('model_name', 'SVM')} - Akurasi: {model_package['model_info'].get('accuracy', 0):.4f}")
    ...
    if uploaded_file is not None:
        try:
            if uploaded_file.name.endswith('.csv'):
                df = pd.read_csv(uploaded_file)
                # Cari kolom teks
                text_columns = [col for col in df.columns if col.lower() in ['text', 'content', 'review', 'comment', 'ulasan']]
                if text_columns:
                    text_column = text_columns[0]
                else:
                    st.error("File CSV harus memiliki kolom teks (misal: text, content, review)")
                    return
            ...
            if st.button("Analisis Semua Teks", type="primary"):
                ...
                for i, text in enumerate(texts):
                    ...
                    try:
                        ...
                        # Skip jika teks kosong setelah preprocessing
                        if not text_no_stopwords.strip():
                            results.append({
                                'No': i+1,
                                'Teks': text[:100] + '...' if len(text) > 100 else text,
                                'Sentimen': 'TIDAK BISA DIPROSES',
                                'Konfidensi': '0.0000',
                ...
                error = sum(1 for r in results if r['Sentimen'] in ['ERROR', 'TIDAK BISA DIPROSES'])
...
def _implementasi_kelola_model():
    ...
    # Bagian 1: Simpan model saat ini jika ada
    st.write("**Simpan Model Saat Ini**")
    if 'model_package' in st.session_state:
        model_package = st.session_state.model_package
        model_info = model_package['model_info']
        
        st.info(f"Model saat ini: {model_info.get('model_name', 'SVM')} - Akurasi: {model_info.get('accuracy', 0):.4f}")
        
        # Pilihan nama file
        default_name = f"best_sentiment_model_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        filename = st.text_input("Nama file model:", value=default_name)
        
        if st.button("Simpan Model ke File", key="save_current_model"):
            try:
                ...
                st.error(f"Error menyimpan model: {str(e)}")
    else:
        st.info("Belum ada model yang di-training untuk disimpan.")
    
    # Bagian 2: List model yang tersedia di sistem
    st.subheader("Model yang Tersedia di Sistem")
    
    # Cari semua file .pkl
    model_files = []
    for root, dirs, files in os.walk("."):
        for file in files:
            if file.endswith('.pkl') and ('model' in file.lower() or 'sentiment' in file.lower()):
    ...
    if model_files:
        ...
        for file in sorted(model_files, reverse=True):
            try:
                ...
                with col1:
                    st.write(f"**{filename_only}**")
                with col2:
                    st.write(f"{file_size:.1f} KB")
                with col3:
                    st.write(modified_time.strftime('%Y-%m-%d %H:%M'))
                with col4:
                    # Tombol untuk menampilkan detail model
                    if st.button("Lihat Detail", key=f"detail_{filename_only}"):
                        try:
                            with open(file, 'rb') as f:
                                loaded_package = pickle.load(f)
                            
                            if 'model_info' in loaded_package:
                                info = loaded_package['model_info']
                                st.write(f"**Detail Model:**")
                                st.write(f"- Nama: {info.get('model_name', 'N/A')}")
                                st.write(f"- Akurasi: {info.get('accuracy', 0):.4f}")
                                st.write(f"- Kernel: {info.get('kernel', 'N/A')}")
                                st.write(f"- Rasio: {info.get('ratio', 'N/A')}")
                                st.write(f"- Dibuat: {info.get('created_at', 'N/A')}")
                        except:
                            st.warning("Tidak bisa membaca detail model")
                    
                    # Tombol untuk memuat model
                    if st.button("Muat Model", key=f"load_{filename_only}"):
                        try:
                            with open(file, 'rb') as f:
                                loaded_package = pickle.load(f)
                            ...
                            # Validasi
                            required_keys = ['model', 'tfidf_vectorizer', 'model_info']
                            if all(key in loaded_package for key in required_keys):
                                st.session_state.model_package = loaded_package
                                st.success(f"Model {filename_only} berhasil dimuat!")
                                st.experimental_rerun()
                            else:
                                st.error("Format model tidak valid")
                        except Exception as e:
                            st.error(f"Error memuat model: {str(e)}")
        ...
        st.info("Belum ada file model yang tersedia di sistem.")
    
    # Bagian 3: Upload model dari pengguna
    st.subheader("Upload Model dari File")
    ...
    if uploaded_model is not None:
        try:
            ...
            file_size = len(uploaded_model.getvalue()) / 1024  # KB
            st.info(f"Ukuran file: {file_size:.2f} KB")
            
            # Simpan sementara dan coba load
            temp_file = "temp_upload_model.pkl"
            with open(temp_file, 'wb') as f:
                f.write(uploaded_model.getvalue())
            ...
            if 'model_info' in loaded_package:
                info = loaded_package['model_info']
                st.success("File model valid!")
                
                # Tampilkan preview informasi
                st.write("**Informasi Model:**")
                st.write(f"- Nama Model: {info.get('model_name', 'N/A')}")
                st.write(f"- Akurasi: {info.get('accuracy', 0):.4f}")
                st.write(f"- Kernel: {info.get('kernel', 'N/A')}")
                st.write(f"- Rasio: {info.get('ratio', 'N/A')}")
                
                # Tombol untuk memuat model ini
                if st.button("Gunakan Model Ini"):
                    st.session_state.model_package = loaded_package
                    st.success("Model berhasil dimuat ke sistem!")
                    st.experimental_rerun()
            else:
                st.error("File tidak mengandung informasi model yang valid")
            
            # Hapus file sementara
            os.remove(temp_file)
            
        except Exception as e:
            st.error(f"Error memproses file model: {str(e)}")
            if os.path.exists("temp_upload_model.pkl"):
                os.remove("temp_upload_model.pkl")
...
def _analisis_fallback(text, model_info):
    ...
    st.warning("Menggunakan analisis fallback karena model tidak tersedia atau error.")
