...
with tab5:
    ...
    if analyze_btn and input_text.strip():
        with st.spinner("Menganalisis sentimen..."):
            ...
            try:
                if use_trained_model and 'model' in st.session_state:
                    ...
                    if hasattr(model, 'decision_function'):
                        ...
                        confidence = min(abs(decision_score) / 2 + 0.5, 1.0)
                    else:
                        confidence = 0.7  # Default
                    
                else:
                    # Gunakan model demo sederhana
                    st.info("Menggunakan model demo...")
                    
                    # Model demo berdasarkan kata kunci
                    positive_keywords = ['bagus', 'baik', 'mantap', 'ramah', 'cepat', 'mudah', 
                                        'profesional', 'memuaskan', 'puas', 'terbaik']
                    negative_keywords = ['buruk', 'jelek', 'telat', 'error', 'mahal', 'lambat',
                                        'tidak', 'kurang', 'cancel', 'kecewa']
                    ...
                    if positive_count > negative_count:
                        prediction = 1
                        confidence = min(positive_count / 10 + 0.5, 1.0)
                    else:
                        prediction = 0
                        confidence = min(negative_count / 10 + 0.5, 1.0)
                ...
                with col1:
                    if sentiment == "POSITIF":
                        st.markdown(f"""
                        <div style='text-align: center; padding: 2rem; background: linear-gradient(135deg, #E8F5E9, #C8E6C9); 
                                  border-radius: 15px; border: 3px solid #4CAF50;'>
                            <h1 style='color: #4CAF50; font-size: 4rem;'>😊</h1>
                            <h2 style='color: #4CAF50; margin: 1rem 0;'>POSITIF</h2>
                            <p style='font-size: 1.2rem; color: #388E3C;'>Ulasan menunjukkan sentimen positif</p>
                        </div>
                        """, unsafe_allow_html=True)
                    else:
                        st.markdown(f"""
                        <div style='text-align: center; padding: 2rem; background: linear-gradient(135deg, #FFEBEE, #FFCDD2); 
                                  border-radius: 15px; border: 3px solid #F44336;'>
                            <h1 style='color: #F44336; font-size: 4rem;'>😞</h1>
                            <h2 style='color: #F44336; margin: 1rem 0;'>NEGATIF</h2>
                            <p style='font-size: 1.2rem; color: #D32F2F;'>Ulasan menunjukkan sentimen negatif</p>
                        </div>
                        """, unsafe_allow_html=True)
                ...
                with col2:
                    ...
                    # Progress bar
                    st.markdown(f"**Tingkat Keyakinan: {confidence:.1%}**")
                    st.progress(confidence)
                    
                    # Interpretasi confidence
                    if confidence >= 0.8:
                        st.success("🔵 **Tinggi**: Analisis sangat yakin dengan hasil")
                    elif confidence >= 0.6:
                        st.warning("🟡 **Sedang**: Analisis cukup yakin dengan hasil")
                    else:
                        st.info("🟠 **Rendah**: Hasil mungkin kurang akurat")
                    
                    # Detail preprocessing
                    with st.expander("🔍 Detail Analisis"):
                        st.write(f"**Teks input:** {input_text}")
                        st.write(f"**Teks setelah preprocessing:** {processed_text}")
                        st.write(f"**Jumlah kata setelah preprocessing:** {len(processed_text.split())}")
                        
                        if 'model' in st.session_state and use_trained_model:
                            st.write(f"**Model digunakan:** SVM ({st.sidebar.selectbox.__defaults__[0]})")
                            st.write(f"**Akurasi model:** {st.session_state.get('accuracy', 0):.2%}")
                
                # Contoh ulasan lain
                st.markdown("### 💡 Contoh Lainnya")
                
                examples = [
                    ("Driver sopan dan pengemudian aman, sangat recommended!", "POSITIF", 0.92),
                    ("Aplikasi crash terus, loading lama sekali, frustrating", "NEGATIF", 0.88),
                    ("Tarif terjangkau, pelayanan memuaskan", "POSITIF", 0.85),
                    ("Customer service tidak responsif, sangat kecewa", "NEGATIF", 0.79),
                    ("GoFood selalu tepat waktu, makanan masih panas", "POSITIF", 0.91)
                ]
                
                for example_text, example_sentiment, example_conf in examples:
                    col1, col2, col3 = st.columns([3, 1, 1])
                    with col1:
                        st.write(f"\"{example_text}\"")
                    with col2:
                        if example_sentiment == "POSITIF":
                            st.markdown('<span class="positive">POSITIF</span>', unsafe_allow_html=True)
                        else:
                            st.markdown('<span class="negative">NEGATIF</span>', unsafe_allow_html=True)
                    with col3:
                        st.write(f"{example_conf:.0%}")
                    st.divider()
                
            except Exception as e:
                st.error(f"Error dalam analisis: {str(e)}")
                st.info("Coba gunakan model demo atau training model terlebih dahulu.")
...
with col1:
    ...
    st.caption("60% positif, 40% negatif")

with col2:
    st.markdown("**🤖 Model**")
    st.caption("Support Vector Machine")
    if 'model_trained' in st.session_state:
        kernel = st.sidebar.selectbox.__defaults__[0]
        st.caption(f"Kernel: {kernel}")

with col3:
    st.markdown("**📈 Performa**")
    if 'accuracy' in st.session_state:
        acc_class = "accuracy-high" if st.session_state['accuracy'] >= 0.85 else "accuracy-medium" if st.session_state['accuracy'] >= 0.75 else "accuracy-low"
        st.markdown(f'<span class="{acc_class}">{st.session_state["accuracy"]:.2%}</span>', unsafe_allow_html=True)
    else:
        st.caption("Belum diukur")

with col4:
    st.markdown("**⚙️ Preprocessing**")
    st.caption("TF-IDF Vectorization")
    st.caption(f"{max_features} fitur")

st.markdown("---")
st.caption("© 2024 Gojek Sentiment Analysis System | Akurasi: 85-92% | Made with Streamlit")